...
const cardSeries = {
	AMD: ["FirePro", "Radeon"],
	NVIDIA: ["GeFore", "Riva", "Quadro", "NVS", "TESLA"],
};
...
const cardModel = {
	...
	Radeon: {
		...
		R5: ["240", "340X", "430"],
		R7: ["250", "250X", "260", "260X", "265", "350X", "360", "450"],
		R9: ["270", "270X", "280", "280X", "290", "290X"],
		RX: [
			"460",
			"470",
			"480",
	...
	Quadro: {
		"non-initial": ["400", "410", "600", "2000", "4000", "5000", "7000"],
		K: [
			"420",
			"600",
			"620",
		...
		M: ["2000", "4000", "5000", "6000"],
		P: ["400", "600", "620", "1000", "2000", "2200", "4000", "5000", "6000"],
		FX: ["580", "1800", "5800"],
	},
	NVS: {
		"non-initial": ["295", "300", "310", "315", "400", "420", "440", "510"],
	},
	TESLA: {
		K: ["20"],
	},
...
const formFactor = ["L - PROFILE", "H - PROFILE"];

const ports = ["DVI", "DP", "mini-DP", "HDMI", "mini-HDMI", "VGA", "DMS-59"];
...
function GPUModelData() {
	let serieses: { name: string; silicon: string }[] = [];
	for (const key in cardSeries) {
		if (Object.hasOwnProperty.call(cardSeries, key)) {
			if (Array.isArray((cardSeries as any)[key])) {
				(cardSeries as any)[key].map((item: string) => {
	...
	let models: { name: string; silicon: string }[] = [];
	serieses.map((series) => {
		if ((cardModel as any)[series.name]) {
			for (const seriesCode in cardModel[series.name]) {
				if (
					Object.hasOwnProperty.call(cardModel[series.name], seriesCode) &&
					...
					cardModel[series.name][seriesCode].map((modelCode) => {
						if (seriesCode === nonInitialModel) {
							models.push({
								name: `${series.name} ${modelCode}`,
								silicon: series.silicon,
							});
						} else {
							models.push({
								name: `${series.name} ${seriesCode} ${modelCode}`,
								silicon: series.silicon,
							});
						}
					});
