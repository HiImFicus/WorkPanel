...
export function createStockByOldData(rawData) {
	...
	if (data["WS"]) {
		...
		if (data["silicon"] === "AMD") {
			if (firstParse.length >= 2) {
				...
				data["model"] =
					"FirePro " + data["WS"].match(/[a-zA-Z]+|[0-9]+/g).join(" ");
			}
		} else {
			if (firstParse.length >= 2) {
				...
				data["model"] =
					"Quadro " + data["WS"].match(/[a-zA-Z]+|[0-9]+/g).join(" ");
			}
		}
	} else {
		...
		if (data["silicon"] === "AMD") {
			if (firstParse.length >= 2) {
				...
				data["model"] =
					"Radeon " +
					data["GEFORCE/RADEON"].match(/[a-zA-Z]+|[0-9]+/g).join(" ");
			}
		} else {
			if (firstParse.length >= 2) {
				if (firstParse[0] === "GF") {
	...
	function parseSameTypePorts(string) {
		...
		if (trimString) {
			let splitStringByLetterAndNumber = trimString
				.match(/[a-zA-Z]+|[0-9]+/g)
				.filter((ele) => ele !== "X" || ele !== "x");
			if (
				splitStringByLetterAndNumber.length >= 2 &&
				isNumeric(splitStringByLetterAndNumber[0])
			) {
				const number = splitStringByLetterAndNumber.shift();
				return (
					number +
					stockSamePortsMultipleSymbolExpress +
					splitStringByLetterAndNumber.join("-")
				);
			}
		}
	...
	data["ports"] = data["PORTS"]
		.split(",")
		.map((ele) => parseSameTypePorts(ele));
	...
	data["partNumbers"] = data["PART #"]
		.split(",")
		.map((ele) => parsePartNumber(ele, data["brand"]), data);

	data["status"] = data["STATUS"]?.toLowerCase();

	if (
		data["STATUS"] &&
		[stockStatusInStock, stockStatusOut].includes(data["STATUS"].toLowerCase())
	) {
		data["status"] = data["STATUS"]?.toLowerCase();
	} else {
...
export async function stockSaveFromAdd(stockData) {
	...
	const hasRecord = await gpuDB.record
		.where("[silicon+brand+model+memory+formFactor+ports+partNumbers]")
		.equals([
			record.silicon,
			record.brand,
			record.model,
